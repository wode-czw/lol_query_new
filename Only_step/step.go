...
func Show_what_you_get(rank_30_info []lcu.GameInfo, czw_champion_map map[int]string, port_token string, account_ID int64) float32 {
	...
	for kk, v := range rank_30_info {
		...
		d_to_m_rate, money_to_damage := Get_MTD_by_gameID_v2(v.GameId, account_ID, port_token)
...
func Get_rank30(rank_30_info []lcu.GameInfo, port_token string, accountID int64, bn, en int) ([]lcu.GameInfo, int, bool) {
	...
	for {
		...
		for _, value := range data_struct.Games.Games { //
			...
			fmt.Println(value.GameCreationDate)
...
func Get_MTD_by_gameID_v2(gameID int64, account_ID int64, port_token string) (float32, float32) {
	...
	var MTD_f32 float32
	var MTD_number float32
	MTD_number = my_damge / my_money
